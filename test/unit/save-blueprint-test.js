...
describe(_saveBlueprint, function () {
  ...
  beforeEach(function () {
    loadDefaultBlueprintFromDisk = sinon
      .stub(utils, 'loadDefaultBlueprintFromDisk')
      .withArgs({ cwd: emberCliUpdateJsonPath })
      .resolves(
        loadSafeBlueprint({
    ...
    loadSafeBlueprintFile = sinon
      .stub(utils, 'loadSafeBlueprintFile')
  ...
  describe('default', function () {
    it('saves default blueprint when missing', async function () {
      ...
      expect(JSON.stringify(saveBlueprintFile.args[0][1])).to.equal(
        JSON.stringify({
          blueprints: [
            loadSafeBlueprint({
              packageName: defaultPackageName,
