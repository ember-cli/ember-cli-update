...
describe(checkForBlueprintUpdates, function () {
  ...
  it('works', async function () {
    ...
    let urlBlueprint = (
      await loadSafeBlueprintFile(
        'test/fixtures/blueprint/app/remote-app/local/my.app/config/ember-cli-update.json'
      )
    ).blueprints[0];
    ...
    let blueprintUpdates = await checkForBlueprintUpdates({
      cwd: tmpPath,
      blueprints: [localBlueprint, urlBlueprint, npmBlueprint]
    });
